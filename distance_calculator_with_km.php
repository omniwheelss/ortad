...
if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'do' && isset($_REQUEST['from']) && isset($_REQUEST['to']) ){
	...
	function Get_Daily_Summary_Sample($Date, $IMEI){
				...
				//foreach($Valid_Records as $Result_Array)
				{
					...
					if($Diff_Record == 0){
						...
						if($Data_Cur_Status_Val  == 'Moving'){
							...
							//$Result_Array['odometer'] = "Odameter--".$Result_Array['odometer'];
					...
					//echo $i."-----".$Result_Array['device_date_stamp']."---".$Result_Array['odometer']."<br />";
		...
		return $Result = array($Speed_Array, $All_DateTime_Diff, $All_DateTime_NE_Diff, $DateTime_Moving_Diff, $DateTime_Stopped_Diff, $DateTime_Idle_Diff, $DateTime_Unknown_Diff, $Decision_Maker_Moving_Diff, $Decision_Maker_Stopped_Diff, $Decision_Maker_Idle_Diff, $Decision_Maker_Unknown_Diff,$Total_KM_Value);
	}

	// Resulting for IMEI 
	foreach($IMEI_Array as $IMEI){
		
		foreach($Date_Array as $Dates_Val){
			
			echo "<br /><u><b>IMEI :".$IMEI. "-------DATE : ". $Dates_Val."</b></u><br />";
			
			// Getting all the data by status
			$Get_Summary = Get_Daily_Summary_Sample($Dates_Val, $IMEI);
			...
			$Total_KM_Travelled = $Get_Summary[11];
		...
		echo "<h4>Total Up Time -- ".$Total_Pocket_Time;
		...
		echo "<br />Unknown Time -- ".$Total_Unknown_Pocket_Time."<br />";
		//echo "<hr />";
		...
		echo "<br />Total KM Travelled -- ".$Total_KM_Travelled."Km";
